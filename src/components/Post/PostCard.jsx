...
const PostCard = ({ post, featured = false, status=true }) => {
  ...
  return (
    <Link
      to={`/posts/${post.slug}`}
      className={`bg-slate-200 rounded-xl dark:bg-slate-900/10 border !border-white/20 group active:scale-[0.99] duration-100 relative flex flex-col overflow-hidden ${
        featured ? "h-full" : ""
      }`}
    >
