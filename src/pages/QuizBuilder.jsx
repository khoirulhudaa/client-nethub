...
export default function QuizBuilder() {
  ...
  const addQuestion = () => {
    if (questions.length >= 20) {
      alert("Maximum 20 questions per quiz");
      return;
    }
  ...
  const handleSave = async (publish = false) => {
    ...
    if (questions.length < 5) {
      setError("Minimum 5 questions");
      return;
    }
    if (questions.length > 15) {
      setError("Maximum 15 questions");
      return;
    }
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.type === "multiple_choice") {
        ...
        if (emptyOption) {
          setError(`Question #${i + 1}: all answer options must be filled`);
          setActiveIndex(i);
          return;
        }
        ...
        if (!hasCorrect) {
          setError(`Question #${i + 1} (Multiple Choice) has no correct answer`);
          setActiveIndex(i);
          return;
        }
      }
      ...
      if (q.type === "topology") {
        if (!q.correctTopology?.nodes?.length) {
          setError(`Question #${i + 1} (Topology) is missing the answer key`);
          setActiveIndex(i);
          return;
        }
      }
    ...
    <div className="mx-auto max-w-full md:border-x border-white dark:border-white/10 space-y-6 pb-16 px-0 py-0 md:py-6 md:px-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={saving}
            className="btn-secondary py-2 md:py-2.5 inline-flex items-center gap-1.5 text-sm"
          >
            <Save size={15} />
            Draft
          </button>
          <button
            ...
            disabled={saving}
            className="btn-primary py-2 md:py-2.5 inline-flex items-center gap-1.5 text-sm"
          >
            <Save size={15} />
      ...
      <div className="gap-4 border border-gray-200 p-3 md:py-7 md:px-4 w-full sm:py-4 relative dark:border-white/5 bg-slate-200 dark:bg-white/5 rounded-xl">
        ...
        <div className="col-span-2 w-full flex flex-col items-center gap-3">
          <div className="w-full">
            ...
            <input
              ...
              placeholder="e.g. Basic Office Topology"
        ...
        <div className="sm:col-span-2">
          ...
          <textarea
            ...
            placeholder="What will be learned in this quiz?"
          />
        </div>
      ...
      <div className="space-y-5">
        ...
        <div className="flex-1 space-y-5 rounded-xl border border-gray-200 dark:border-white/5 bg-slate-200 dark:bg-white/5 p-3 md:p-5">
          <div className="flex items-center justify-between">
            <h3 className="font-medium">Question #{activeIndex + 1}</h3>
            <button
              type="button"
          ...
          <div className="flex gap-3">
            ...
            >
              <CheckSquare size={16} /> Multiple Choice
            </button>
            <button
              type="button"
            ...
            >
              <Network size={16} /> Topology Practice
            </button>
          </div>
          ...
          {current.type === "topology" && (
            <div className="space-y-4">
              <div className="rounded-lg bg-blue-50 p-3 text-sm text-blue-800 dark:bg-blue-500/10 dark:text-blue-300">
                <p className="font-medium">Topology Answer Key</p>
                <p className="mt-1 text-sm md:text-xs">
                  Build the correct topology below. Users must match the same
                  hardware layout and cable types.
                </p>
              </div>
